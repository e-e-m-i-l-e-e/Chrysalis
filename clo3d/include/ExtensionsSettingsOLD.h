...
namespace Ui {
    class ExtensionsSettingsOLD;
}

QT_END_NAMESPACE

class ExtensionsSettingsOLD : public QDialog {
    Q_OBJECT
public:

    explicit ExtensionsSettingsOLD(QWidget *parent = nullptr);
    ~ExtensionsSettingsOLD() override;
...
private:
    ...
    struct Shortcut : QObject {
        explicit Shortcut(ExtensionsSettingsOLD *parent);
        ~Shortcut() override;
